...
var ToutiaoWS = /** @class */ (function (_super) {
    __extends(ToutiaoWS, _super);
    function ToutiaoWS(url, protocol) {
        var _this = _super.call(this, url, protocol) || this;
        _this._readyState = WS.CONNECTING;
        var protocols = [];
        if (protocol) {
            var str = void 0; // protocol string
            ...
            var sp = _this._url.includes("?") ? "&" : "?";
            _this._url += sp + "subprotocol=" + str;
        }
        _this._socketTask = tt.connectSocket({ url: _this._url, protocols: protocols });
        _this._socketTask.onOpen(function () {
            _this._readyState = WS.OPEN;
            _this.dispatchEvent({ type: "open" });
        });
        _this._socketTask.onError(function (err) {
            _this.dispatchEvent({ type: "error", message: err.errMsg });
            _this.close();
        });
        _this._socketTask.onMessage(function (msg) {
            _this.dispatchEvent({ type: "message", data: msg.data });
        });
        _this._socketTask.onClose(function () {
            _this._readyState = WS.CLOSED;
            _this.dispatchEvent({ type: "close" });
        });
        return _this;
    }
    ToutiaoWS.prototype.send = function (data) {
    ...
    ToutiaoWS.prototype.close = function () {
...
var upload = function (url, file, options) {
    ...
    return new Promise(function (resolve, reject) {
        var task = tt.uploadFile({
            ...
            fail: function (err) { return reject(new Error(err.errMsg)); },
            complete: function () {
                signal === null || signal === void 0 ? void 0 : signal.removeEventListener("abort", abortListener);
            },
        });
